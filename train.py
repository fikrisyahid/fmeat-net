...
def main(model_type, pso, mp_mode):
    ...
    def train_model(x):
        ...
        for particle_index in range(len(x) if using_pso else 1):
            ...
            learning_rate = x[:, 0][particle_index] if using_pso else config.LEARNING_RATE
            batch_size = (
                helper.round_to_closest_possible(config.PSO_BATCH_SIZE, x[:, 1][particle_index]) if using_pso else config.BATCH_SIZE
            )
            fine_tune_unfreeze_layer = (
                helper.round_to_closest_possible(config.PSO_FINE_TUNE_UNFREEZE_LAYER, x[:, 2][particle_index])
                if using_pso
                else config.FINE_TUNE_UNFREEZE_LAYER
            )
            dropout_rate = x[:, 3][particle_index] if using_pso else config.DROPOUT_RATE

            if using_pso:
                print(f"Final learning rate: {learning_rate}")
                print(f"Final batch size: {batch_size}")
                print(f"Final fine tune unfreeze layer: {fine_tune_unfreeze_layer}")
            ...
            model = models.CNNModel(dropout_rate) if model_type == "cnn" else models.VGGModel(dropout_rate)
            ...
            testing_accuracies.append(-test_accuracy if using_pso else test_accuracy)
    ...
    if pso:
        ...
        bounds = (
            [
                0.0001 if model_type == "cnn" else 0.000001,  # Lower bound untuk learning rate
                16,  # Lower bound untuk batch size
                0,  # Lower bound untuk fine tune layer
                0.1,  # Lower bound untuk dropout rate
            ],
            [
                0.1 if model_type == "cnn" else 0.001,  # Upper bound untuk learning rate
        ...
        optimizer = GlobalBestPSO(
            ...
            bounds=bounds,
            init_pos=np.array(
                [[0.0001, 16, 0, 0.5], [0.001, 32, 1, 0.2], [0.005, 64, 2, 0.3], [0.007, 128, 3, 0.6], [0.0007, 200, 4, 0.7]]
            ),
        )
